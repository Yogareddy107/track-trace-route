...
import {
  Package, MapPin, CheckCircle, Clock, Navigation, Pencil, Trash2
} from "lucide-react";
...
const AgentDashboard = () => {
  ...
  const handleDeletePackage = (packageId: string) => {
    setAssignedPackages(prev => prev.filter(pkg => pkg.id !== packageId));
  };
  ...
  const handleEditPackage = (packageId: string) => {
    ...
    if (newReceiver) {
      setAssignedPackages(prev =>
        prev.map(pkg =>
          pkg.id === packageId ? { ...pkg, receiver: newReceiver } : pkg
        )
      );
    }
  };
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">
                      {assignedPackages.filter(p => p.status === 'pending').length}
                    </p>
                  </div>
                  ...
                  <div key={pkg.id} className="p-4 border-b hover:bg-gray-50">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-1"><strong>To:</strong> {pkg.receiver}</p>
                        <p className="text-sm text-gray-600 mb-1"><strong>Address:</strong> {pkg.address}</p>
                        <p className="text-sm text-gray-600"><strong>ETA:</strong> {pkg.eta}</p>
                      </div>
                    </div>
